...
def test_quick_tests():
    ...
    netw = epj.EJson.read_from_file(test_netws_path / 'netw_generic_a.json')

    assert netw.component('nd1')['id'] == 'nd1'
    ...
    netw_trim: epj.EJson = netw.clone()
    netw_trim.trim('ln10_11', stop_cb=lambda netw, c: c['id'] == 'nd10')
    assert len(list(netw_trim.components())) == len(list(netw.components())) - 4

    netw_trim: epj.EJson = netw.clone()
    netw_trim.trim('nd4', stop_cb=lambda netw, c: c['id'] == 'nd6')
    assert len(list(netw_trim.components())) == len(list(netw.components())) - 11

    netw_only: epj.EJson = netw.clone()
    netw_only.only('nd5', stop_cb=lambda netw, c: c['id'] == 'ln5_6')
    assert len(list(netw_only.components())) == 10

    netw_recon: epj.EJson = netw.clone()
    netw_recon.reconnect_elem('ld8', {'nd8': 'nd4'})
    assert next(iter(netw_recon.connections_from('ld8')))[1] == 'nd4'

    netw_rename: epj.EJson = netw.clone()
    netw_rename.rename()
    assert [c['id'] for c in netw_rename.components()] == [
...
def test_collapse():
    netw = epj.EJson.read_from_file(test_netws_path / 'netw_test_collapse.json')
    epj.collapse_elem(netw, 'ln3_4')
    assert set((x['id'] for x in netw.components())) == set(
        ('in1', 'nd1', 'tx1_2', 'nd2', 'ln2_3', 'nd3', 'ln4_5', 'nd5', 'ld5')
    )

    connected = [x[1] for x in netw.connections_from('ln4_5')]
    assert connected == ['nd3', 'nd5']


def test_reorder():
    netw = epj.EJson.read_from_file(test_netws_path / 'netw_test_reorder.json')
    netw.reorder('ld3')
    assert [x['id'] for x in netw.components()] == ['ld3', 'nd3', 'ln2_3', 'nd2', 'tx1_2', 'nd1', 'in1']
...
def test_make_radial():
    netw = epj.EJson.read_from_file(test_netws_path / 'netw_test_make_radial.json')
    epj.make_radial(netw, 'in1')
    assert [x['id'] for x in netw.components()] == ['in1', 'nd1', 'tx1_2', 'nd2', 'ln2_3', 'nd3', 'ln3_4', 'nd4']


def test_remove_unconnected():
    netw = epj.EJson.read_from_file(test_netws_path / 'netw_test_remove_unconnected.json')
...
def test_set_loads():
    netw = epj.EJson.read_from_file(test_netws_path / 'netw_generic_a.json')
    epj.set_balanced_loads(netw, 12.0 + 3.0j)
    assert netw.component('ld8')['s_nom'] == [[4.0, 1.0], [4.0, 1.0], [4.0, 1.0]]
    epj.scale_loads(netw, 2.0)
    assert netw.component('ld8')['s_nom'] == [[8.0, 2.0], [8.0, 2.0], [8.0, 2.0]]


def test_make_single_phased():
    netw = epj.EJson.read_from_file(test_netws_path / 'netw_test_make_single_phased.json')
    netw_sp = netw.clone()

    epj.make_single_phased(netw_sp)
    ...
    for tx in netw_sp.components(ctype='Transformer'):
        assert tx['vector_group'] == 'yy0'

    assert netw_sp.component('ld8')['s_nom'] == [[1.0, 1.0]]
    assert netw_sp.component('ld9')['s_nom'] == [[3.0, 1.0]]

    def close(a, b):
        return abs(complex(*a) - complex(*b)) < 1e-9

    assert close(netw_sp.component('ln2_3')['z'], [1.0, 1.1])
    assert close(netw_sp.component('ln2_3')['z0'], [1.0, 1.1])

    assert close(netw_sp.component('ln6_8')['z'], [6.0, 0.0])
    assert close(netw_sp.component('ln6_8')['z0'], [6.0, 0.0])

    assert close(netw_sp.component('ln11_12')['z'], [3.0, 3.15])
    assert close(netw_sp.component('ln11_12')['z0'], [3.0, 3.15])


def test_reduce_merge_strings():
    ''' Test reduce where there is a string of lines to merge.'''
    netw = epj.EJson.read_from_file(test_netws_path / 'netw_test_reduce_merge_strings.json')

    def get_imp(netw, z):
        return sum((complex(*x[z]) * x['length']) for x in netw.components('Line'))
    ...
    assert sum((x['length'] for x in netw.components('Line'))) == 3
...
def test_reduce_remove_hanging():
    ...
    netw = epj.EJson.read_from_file(test_netws_path / 'netw_test_reduce_remove_hanging.json')
    assert set(x['id'] for x in netw.components()) == expected_before
    epj.reduce_network(netw)
    assert set(x['id'] for x in netw.components()) == expected_after
...
def test_reduce_merge_shorts():
    ...
    netw = epj.EJson.read_from_file(test_netws_path / 'netw_test_reduce_merge_shorts.json')
    assert set(x['id'] for x in netw.components()) == expected_before
    epj.reduce_network(netw)
    assert set(x['id'] for x in netw.components()) == expected_after
...
def test_reduce_merge_dups():
    netw = epj.EJson.read_from_file(test_netws_path / 'netw_test_reduce_merge_dups.json')
    epj.reduce_network(netw)
    lines = list(netw.components('Line'))
    assert len(lines) == 1
    line = lines[0]
    assert line['z'] == [0.5, 0.5]
    assert line['z0'] == [0.5, 0.5]


def test_reduce():
    netw = epj.EJson.read_from_file(test_netws_path / 'netw_test_reduce.json')
...
def test_audit():
    ...
    for section in aud.values():
        assert len(section['problems']) == 0, f'Problems: {section["problems"]}'
